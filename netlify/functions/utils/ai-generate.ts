/**
 * Движок ИИ-генерации: сборка системного промпта по типу запроса, чтение
 * приложенного файла (PDF/картинка из Storage) и вызов Gemini.
 *
 * Живёт отдельно от хендлера, потому что входов у генерации два:
 *  - `api-ai-generate` — синхронный, для коротких текстовых операций
 *    (перевод, комментарий к отчёту, пост), они укладываются в лимит Netlify;
 *  - `api-ai-generate-background` — фоновый, для тяжёлых (экзамен/викторина по
 *    лекции, урок+квиз). Замер на реальном материале: 15 вопросов по тексту
 *    лекции — 17.7 с, то есть синхронная функция гарантированно ловит 504.
 */
import { generateWithFallback, parseJsonLoose } from './ai';

/** Ошибка генерации с разделением «виноват ввод» / «виновал сервер». */
export class AIGenerateError extends Error {
  constructor(
    message: string,
    public readonly kind: 'file' | 'format' | 'server' = 'server',
...
/** Собирает промпт, читает файл, зовёт Gemini и возвращает разобранный JSON. */
export async function generateAIContent({ prompt, type = 'quiz', fileUrl }: AIGenerateInput): Promise<any> {
  const parts: any[] = [];
...
Do not include any extra text, only the JSON array.`;
  ...
  if (fileUrl) {
    try {
      ...
      throw new AIGenerateError(`Не удалось прочитать приложенный файл: ${fileErr.message}`, 'file');
    }
  }

  const result = await generateWithFallback({ json: true }, parts);
  const textResp = result.response.text();

  try {
    return parseJsonLoose(textResp);
  } catch {
    console.error('Failed to parse Gemini JSON output:', textResp);
    throw new AIGenerateError('ИИ вернул ответ в неверном формате', 'format', textResp);
  }
}
