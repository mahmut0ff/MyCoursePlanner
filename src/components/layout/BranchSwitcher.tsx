...
const BranchSwitcher: React.FC<BranchSwitcherProps> = ({ onClose, isCollapsed }) => {
  ...
  if (loading) {
    return (
      <div className={`px-3 py-2 ${isCollapsed ? 'lg:flex lg:justify-center lg:px-0' : ''}`}>
        <div className={`h-10 bg-slate-100 dark:bg-white/5 rounded-xl animate-pulse ${isCollapsed ? 'lg:w-10' : ''}`} />
      </div>
    );
  }
  ...
  return (
    <div ref={rootRef} className={`relative px-3 py-2 ${isCollapsed ? 'lg:flex lg:justify-center lg:px-0' : ''}`}>
      <button
      ...
      >
        ...
        <div className={`flex items-center gap-2.5 flex-1 min-w-0 ${isCollapsed ? 'lg:hidden' : ''}`}>
          <div className="flex-1 min-w-0">
            <p className="text-[13px] text-slate-900 dark:text-white truncate font-semibold leading-tight">{label}</p>
            <p className="text-[10px] text-violet-600 dark:text-violet-400/80 font-medium mt-0.5 truncate">{subLabel}</p>
          </div>
