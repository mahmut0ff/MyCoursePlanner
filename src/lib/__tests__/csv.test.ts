...
describe('escapeCsvCell — числа', () => {
  // Штраф в ведомости — отрицательное ЧИСЛО. Защита от формул к нему
  // неприменима: с ведущим апострофом таблица считает ячейку текстом, колонка
  // перестаёт суммироваться, и сверить выгрузку с экраном становится нельзя.
  it('не превращает отрицательное число в текст', () => {
    expect(escapeCsvCell(-5000)).toBe('-5000');
    expect(escapeCsvCell(-5000.25)).toBe('-5000.25');
  });

  it('обычные числа тоже проходят без изменений', () => {
    expect(escapeCsvCell(0)).toBe('0');
    expect(escapeCsvCell(30000)).toBe('30000');
  });

  // Послабление касается только настоящих чисел: пользовательский ввод строкой
  // по-прежнему обезврежен.
  it('строку с ведущим минусом по-прежнему экранирует', () => {
    expect(escapeCsvCell('-500')).toBe("'-500");
    expect(escapeCsvCell('=1+1')).toBe("'=1+1");
  });
});
