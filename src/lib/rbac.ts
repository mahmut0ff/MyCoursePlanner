  ...
  },
  {
    ...
    resources: [
      ...
      { id: 'payroll', label: 'Зарплата', help: {
        ...
        write: 'Ставки, расчёт месяца, утверждение и выплата',
        delete: 'Удаление ставок и ручных строк расчёта',
        // Зарплата общеорганизационная: филиалом это право не сужается — расчёт
        // идёт по всей организации, а по филиалам раскладывается уже начисленное.
        notes: 'Даётся по всей организации: филиалом не ограничивается. Доступно на тарифе Профессиональный и выше.',
      } },
      ...
      // Read-only: журнал действий преподавателей и их KPI. Отдельно от «Аналитики»
      // (это успеваемость учеников) — здесь надзор за работой персонала, и директор
      // может выдать его, не открывая финансы/аналитику целиком.
      { id: 'teacher_activity', label: 'Активность преподавателей (KPI)', actions: ['read'], help: {
        read: 'Кто ставит оценки и посещаемость, проверяет ДЗ, создаёт экзамены и уроки, когда заходил — сводка и KPI по преподавателям',
        notes: 'Только просмотр. Активность фиксируется автоматически на сервере в момент самих действий — накрутить её с клиента нельзя.',
      } },
    ],
  },
  {
