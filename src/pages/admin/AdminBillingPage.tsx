    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-6">
        <div className="bg-white border border-slate-200 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-1"><DollarSign className="w-4 h-4 text-emerald-500" /><span className="text-xs text-slate-500">MRR</span></div>
          <p className="text-2xl font-bold text-slate-900">${totalMRR}</p>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-1"><CreditCard className="w-4 h-4 text-primary-500" /><span className="text-xs text-slate-500">Active</span></div>
          <p className="text-2xl font-bold text-slate-900">{activeCount}</p>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-1"><Clock className="w-4 h-4 text-amber-500" /><span className="text-xs text-slate-500">Trial</span></div>
          <p className="text-2xl font-bold text-slate-900">{trialCount}</p>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-1"><Ban className="w-4 h-4 text-red-500" /><span className="text-xs text-slate-500">Cancelled</span></div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
        <table className="w-full">
          <thead className="bg-slate-50 border-b"><tr>
            ...
            <th className="text-left px-4 py-3 text-xs font-medium text-slate-500 uppercase">Period End</th>
            <th className="text-left px-4 py-3 text-xs font-medium text-slate-500 uppercase">Actions</th>
          </tr></thead>
          <tbody className="divide-y">
            {filtered.map((s) => (
              <tr key={s.id} className="hover:bg-slate-50">
                <td className="px-4 py-3"><p className="text-sm font-medium text-slate-900">{s.organizationName}</p><p className="text-xs text-slate-400">{s.organizationId?.slice(0, 12)}...</p></td>
                ...
                <td className="px-4 py-3 text-xs text-slate-500">{s.currentPeriodEnd ? new Date(s.currentPeriodEnd).toLocaleDateString() : '—'}</td>
            ...
            {filtered.length === 0 && <tr><td colSpan={6} className="text-center py-12 text-slate-400 text-sm">{loading ? 'Loading...' : 'No subscriptions found'}</td></tr>}
          </tbody>
        </table>
      </div>
      ...
      {extendModal && (
        <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center p-4" onClick={() => setExtendModal(null)}>
          <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-xl" onClick={(e) => e.stopPropagation()}>
            <h3 className="font-semibold text-slate-900 mb-4">Extend Subscription</h3>
            <p className="text-sm text-slate-500 mb-4">{extendModal.name}</p>
            <label className="text-xs font-medium text-slate-500">Days to extend</label>
