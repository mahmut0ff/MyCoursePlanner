  ...
  return (
    <div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden mb-8">
        <div className="px-6 py-4 border-b"><h3 className="font-semibold text-slate-900 flex items-center gap-2"><Settings className="w-4 h-4" />Global Feature Flags</h3></div>
        <div className="divide-y">
          {flags.map((f) => (
            <div key={f.id} className="px-6 py-4 flex items-center gap-4 hover:bg-slate-50">
              ...
              <div className="flex-1">
                <p className="text-sm font-medium text-slate-900 font-mono">{f.key}</p>
                <p className="text-xs text-slate-500">{f.description || 'No description'}</p>
              </div>
              <span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${f.enabled ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>{f.enabled ? 'ON' : 'OFF'}</span>
            </div>
          ))}
          {flags.length === 0 && <div className="text-center py-8 text-slate-400 text-sm">No feature flags configured</div>}
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
        <div className="px-6 py-4 border-b flex items-center justify-between">
          <h3 className="font-semibold text-slate-900 flex items-center gap-2"><Building2 className="w-4 h-4" />Organization Overrides</h3>
        ...
        <table className="w-full">
          <thead className="bg-slate-50 border-b"><tr>
            ...
            <th className="text-left px-4 py-3 text-xs font-medium text-slate-500 uppercase">Max Students</th>
            <th className="text-left px-4 py-3 text-xs font-medium text-slate-500 uppercase">Max Teachers</th>
            <th className="text-left px-4 py-3 text-xs font-medium text-slate-500 uppercase">Max Exams</th>
            <th className="text-left px-4 py-3 text-xs font-medium text-slate-500 uppercase">AI</th>
          </tr></thead>
          <tbody className="divide-y">
            {overrides.map((o) => {
              ...
              return (
                <tr key={o.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-sm text-slate-600">{o.maxStudents ?? '—'}</td>
                  <td className="px-4 py-3 text-sm text-slate-600">{o.maxTeachers ?? '—'}</td>
                  <td className="px-4 py-3 text-sm text-slate-600">{o.maxExams ?? '—'}</td>
            ...
            {overrides.length === 0 && <tr><td colSpan={5} className="text-center py-8 text-slate-400 text-sm">No overrides configured</td></tr>}
          </tbody>
        </table>
      </div>
      ...
      {showNewFlag && (
        <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center p-4" onClick={() => setShowNewFlag(false)}>
          <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-xl" onClick={(e) => e.stopPropagation()}>
            <h3 className="font-semibold text-slate-900 mb-4">New Feature Flag</h3>
            <div className="space-y-3">
              <div><label className="text-xs font-medium text-slate-500">Key</label><input type="text" value={newFlag.key} onChange={(e) => setNewFlag({ ...newFlag, key: e.target.value })} className="input text-sm" placeholder="enable_ai_v2" /></div>
              <div><label className="text-xs font-medium text-slate-500">Description</label><input type="text" value={newFlag.description} onChange={(e) => setNewFlag({ ...newFlag, description: e.target.value })} className="input text-sm" placeholder="Enable AI v2 features" /></div>
            </div>
      ...
      {showOverride && (
        <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center p-4" onClick={() => setShowOverride(false)}>
          <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-xl" onClick={(e) => e.stopPropagation()}>
            <h3 className="font-semibold text-slate-900 mb-4">Organization Override</h3>
            <div className="space-y-3">
              <div><label className="text-xs font-medium text-slate-500">Organization</label>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div><label className="text-xs font-medium text-slate-500">Max Students</label><input type="number" value={overrideData.maxStudents} onChange={(e) => setOverrideData({ ...overrideData, maxStudents: e.target.value })} className="input text-sm" /></div>
                <div><label className="text-xs font-medium text-slate-500">Max Teachers</label><input type="number" value={overrideData.maxTeachers} onChange={(e) => setOverrideData({ ...overrideData, maxTeachers: e.target.value })} className="input text-sm" /></div>
                <div><label className="text-xs font-medium text-slate-500">Max Exams</label><input type="number" value={overrideData.maxExams} onChange={(e) => setOverrideData({ ...overrideData, maxExams: e.target.value })} className="input text-sm" /></div>
              </div>
              <div><label className="text-xs font-medium text-slate-500">AI Access</label>
