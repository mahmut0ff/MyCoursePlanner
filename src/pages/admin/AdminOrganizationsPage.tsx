...
const STATUS_COLORS: Record<string, string> = { active: 'bg-emerald-100 text-emerald-700', suspended: 'bg-red-100 text-red-700', trial: 'bg-amber-100 text-amber-700', deleted: 'bg-slate-100 text-slate-500' };
...
const AdminOrganizationsPage: React.FC = () => {
  ...
  return (
    <div className="flex h-[calc(100vh-120px)]">
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y">
              {data.organizations.map((org) => (
                <tr key={org.id} className="hover:bg-slate-50 cursor-pointer" onClick={() => openDetail(org.id)}>
                  ...
                  <td className="px-4 py-3 text-xs text-slate-500">{new Date(org.createdAt).toLocaleDateString()}</td>
                  <td className="px-4 py-3">
                    <div className="flex gap-2" onClick={(e) => e.stopPropagation()}>
                      ...
                      <button onClick={() => openDetail(org.id)} className="text-slate-400 hover:text-slate-600"><ChevronRight className="w-4 h-4" /></button>
                    </div>
                  </td>
                </tr>
              ))}
              {data.organizations.length === 0 && <tr><td colSpan={5} className="text-center py-12 text-slate-400 text-sm">{loading ? 'Loading...' : 'No organizations found'}</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
      ...
      {/* Right: Detail Panel */}
      {selected && (
        <div className="w-full lg:w-[440px] lg:ml-6 bg-white border border-slate-200 rounded-xl overflow-hidden flex flex-col">
            ...
            {detailLoading ? <div className="flex items-center justify-center py-8"><div className="w-6 h-6 border-2 border-primary-200 border-t-primary-600 rounded-full animate-spin" /></div> : (
              <>
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  ...
                  <div><p className="text-slate-500 text-xs">Owner</p><p className="font-medium text-slate-900 truncate">{selected.ownerEmail}</p></div>
                ...
                {selected.usage && (
                  <div>
                    <h4 className="text-xs font-semibold text-slate-400 uppercase mb-2">Usage</h4>
                    <div className="grid grid-cols-3 gap-2">
                      {[
                        ...
                        <div key={u.label} className="bg-slate-50 rounded-lg px-3 py-2 text-center">
                          <p className="text-lg font-bold text-slate-900">{u.value}</p>
                          <p className="text-[10px] text-slate-500">{u.label}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {/* Plan Change */}
                <div>
                  <h4 className="text-xs font-semibold text-slate-400 uppercase mb-2">Change Plan</h4>
                  <div className="flex gap-2">
                    {['starter', 'professional', 'enterprise'].map((p) => (
                      <button key={p} disabled={selected.planId === p} onClick={() => handleChangePlan(selected.id, p)}
                        className={`flex-1 px-3 py-2 rounded-lg text-xs font-medium transition-colors ${selected.planId === p ? 'bg-primary-100 text-primary-700 cursor-not-allowed' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                        {p}
                      </button>
                    ))}
                  </div>
                ...
                {/* Actions */}
                <div>
                  <h4 className="text-xs font-semibold text-slate-400 uppercase mb-2">Actions</h4>
                ...
                {/* Notes */}
                <div>
                  <h4 className="text-xs font-semibold text-slate-400 uppercase mb-2">Internal Notes</h4>
                ...
                {selected.subscription && (
                  <div>
                    <h4 className="text-xs font-semibold text-slate-400 uppercase mb-2">Subscription</h4>
