...
const AdminSystemHealthPage: React.FC = () => {
  ...
  if (!health) return <div className="text-center py-20 text-slate-400 dark:text-slate-500">Failed to load health data</div>;
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-500 text-sm">Monitor platform services and performance</p>
        </div>
      ...
      <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-6 mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-900 dark:text-white capitalize">All Systems {health.status}</h2>
            <p className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500">Uptime: {health.uptime} · Average Latency: {health.apiLatency}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-2"><Cpu className="w-4 h-4 text-primary-500" /><span className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500">Active Functions</span></div>
          <p className="text-2xl font-bold text-slate-900">{health.activeFunctions}</p>
        </div>
        <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-2"><Activity className="w-4 h-4 text-emerald-500" /><span className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500">Actions (24h)</span></div>
          <p className="text-2xl font-bold text-slate-900">{health.last24hActions}</p>
        </div>
        <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-2"><Wifi className="w-4 h-4 text-violet-500" /><span className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500">API Latency</span></div>
          <p className="text-2xl font-bold text-slate-900">{health.apiLatency}</p>
        </div>
      </div>

      {/* Services */}
      <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden mb-6">
        <div className="px-6 py-4 border-b"><h3 className="font-semibold text-slate-900 dark:text-white flex items-center gap-2"><Server className="w-4 h-4" />Services</h3></div>
      ...
      <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden">
        <div className="px-6 py-4 border-b"><h3 className="font-semibold text-slate-900 dark:text-white flex items-center gap-2"><AlertTriangle className="w-4 h-4 text-red-500" />Recent Errors</h3></div>
        <div className="divide-y divide-slate-100 dark:divide-slate-700">
          ...
          {(health.recentErrors || []).length === 0 && <div className="text-center py-8 text-emerald-500 text-sm flex items-center justify-center gap-2"><CheckCircle2 className="w-4 h-4" />No recent errors</div>}
        </div>
      </div>
    </div>
  );
