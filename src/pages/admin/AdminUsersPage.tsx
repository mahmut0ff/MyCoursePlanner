...
const AdminUsersPage: React.FC = () => {
  ...
  const handleDisable = async (uid: string) => { if (!confirm(t('admin.users.confirmDisable'))) return; await adminDisableUser(uid); openDetail(uid); load(); };
            ...
            {detailLoading ? <div className="flex items-center justify-center py-8"><div className="w-6 h-6 border-2 border-primary-200 border-t-primary-600 rounded-full animate-spin" /></div> : (
              <>
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div><p className="text-slate-500 dark:text-slate-400 text-xs">{t('admin.users.role')}</p><span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${ROLE_COLORS[selected.role] || ''}`}>{selected.role}</span></div>
                  <div><p className="text-slate-500 dark:text-slate-400 text-xs">{t('admin.users.status')}</p><span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${selected.disabled ? 'bg-red-100 text-red-700' : 'bg-emerald-100 text-emerald-700'}`}>{selected.disabled ? t('common.disabled') : t('common.active')}</span></div>
                  <div><p className="text-slate-500 dark:text-slate-400 text-xs">{t('admin.users.organization')}</p><p className="font-medium text-slate-900 dark:text-white">{selected.organizationName || '—'}</p></div>
                  <div><p className="text-slate-500 dark:text-slate-400 text-xs">{t('admin.users.created')}</p><p className="font-medium text-slate-900 dark:text-white">{new Date(selected.createdAt).toLocaleDateString()}</p></div>
                </div>

                <div>
                  <h4 className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase mb-2">{t('admin.users.changeRole')}</h4>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selected.disabled ? (
                      <button onClick={() => handleEnable(selected.uid)} className="btn-primary text-xs flex items-center gap-1"><Check className="w-3 h-3" />{t('admin.users.enable')}</button>
                    ) : (
                      <button onClick={() => handleDisable(selected.uid)} className="btn-secondary text-xs flex items-center gap-1"><Ban className="w-3 h-3" />{t('admin.users.disable')}</button>
                    )}
                    <button onClick={() => handleReset(selected.email)} className="btn-secondary text-xs flex items-center gap-1"><Key className="w-3 h-3" />{t('admin.users.resetPassword')}</button>
                  </div>
                </div>

                {selected.recentAttempts?.length > 0 && (
                  <div>
                    <h4 className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase mb-2">{t('admin.users.recentExams')}</h4>
                    <div className="space-y-2">
                      {selected.recentAttempts.slice(0, 5).map((a: any) => (
                        <div key={a.id} className="bg-slate-50 dark:bg-slate-700/50 rounded-lg px-3 py-2 text-sm">
                          <p className="font-medium text-slate-900 dark:text-white">{a.examTitle}</p>
