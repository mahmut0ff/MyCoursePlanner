  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Here's your learning overview</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        ...
        <Link to="/lessons" className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-6 hover:shadow-lg hover:shadow-slate-200/50 dark:hover:shadow-slate-900/30 transition-all group text-center">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">{lessons.length} available</p>
        </Link>
        <Link to="/my-results" className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-6 hover:shadow-lg hover:shadow-slate-200/50 dark:hover:shadow-slate-900/30 transition-all group text-center">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">{attempts.length} attempts</p>
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
            ...
            <Link to="/my-results" className="text-sm text-primary-600 dark:text-primary-400 hover:text-primary-700 flex items-center gap-1">View all<ArrowRight className="w-3.5 h-3.5" /></Link>
          </div>
          <div className="divide-y divide-slate-100 dark:divide-slate-700">
            {attempts.slice(0, 5).map((a) => (
              <Link key={a.id} to={`/results/${a.id}`} className="block px-5 py-3 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                ...
                <div className="flex items-center gap-2 mt-1 ml-6">
                  {a.aiFeedback && <span className="flex items-center gap-1 text-xs text-primary-600 dark:text-primary-400"><Brain className="w-3 h-3" />AI feedback</span>}
                  <span className="text-xs text-slate-400 dark:text-slate-500">{formatDate(a.submittedAt)}</span>
                </div>
              </Link>
            ))}
            {attempts.length === 0 && <div className="px-5 py-6 text-center text-slate-400 dark:text-slate-500 text-sm">No exams taken yet</div>}
          </div>
        </div>
      </div>
    </div>
