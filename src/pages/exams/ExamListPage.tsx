    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">{exams.length} exams total</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search exams..." className="input pl-10 w-64" />
          </div>
          <Link to="/exams/new" className="btn-primary flex items-center gap-2"><Plus className="w-4 h-4" />New Exam</Link>
        </div>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="card p-12 text-center">
          ...
          <h3 className="text-lg font-medium text-slate-700 mb-1">No exams found</h3>
          <p className="text-slate-500 text-sm">Create your first exam to get started.</p>
          <Link to="/exams/new" className="btn-primary inline-flex items-center gap-2 mt-4"><Plus className="w-4 h-4" />Create Exam</Link>
        </div>
      ) : (
              ...
              {filtered.map((exam) => (
                <tr key={exam.id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-6 py-4">
                    <Link to={`/exams/${exam.id}`} className="font-medium text-slate-900 hover:text-primary-600">{exam.title}</Link>
                  </td>
                  <td className="px-6 py-4 text-sm text-slate-600">{exam.subject}</td>
                  <td className="px-6 py-4 text-sm text-slate-600">{exam.questionCount || 0}</td>
                  <td className="px-6 py-4 text-sm text-slate-600">{exam.durationMinutes} min</td>
                  <td className="px-6 py-4"><span className={statusBadge(exam.status)}>{exam.status}</span></td>
                  <td className="px-6 py-4 text-sm text-slate-500">{formatDate(exam.createdAt)}</td>
                </tr>
              ))}
            </tbody>
          </table>
