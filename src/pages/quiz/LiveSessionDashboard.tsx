...
const LiveSessionDashboard: React.FC = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto kahoot-font">
      ...
      {/* Control Buttons for Active Game */}
      {!isLobby && (
        <div className="flex flex-wrap items-center justify-between gap-2 mb-5 bg-white dark:bg-slate-800 p-3 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
          <div className="flex items-center gap-2">
            {isPlaying && (
              <>
                <button onClick={() => doAction(() => apiNextQuestion(sessionId!), 'next')} disabled={!!actionLoading}
                  className="flex items-center gap-2 px-5 py-2 rounded-lg font-bold text-white text-sm disabled:opacity-40 active:scale-[0.98]"
                ...
                <button onClick={() => doAction(() => apiPauseQuizSession(sessionId!), 'pause')} disabled={!!actionLoading}
                  className="flex items-center gap-2 px-4 py-2 rounded-lg font-bold text-white text-sm disabled:opacity-40"
            ...
            {(isPlaying || isPaused) && (
              <button onClick={handleEnd} disabled={!!actionLoading}
                className="flex items-center gap-2 px-4 py-2 rounded-lg font-bold text-white text-sm disabled:opacity-40 hover:opacity-90"
            ...
            {isCompleted && (
              <button onClick={() => doAction(() => apiRestartQuizSession(sessionId!), 'restart')} disabled={!!actionLoading}
                ...
                style={{ backgroundColor: 'var(--kahoot-purple)', boxShadow: '0 4px 0 rgba(0,0,0,0.2)' }}>
          ...
          <div>
            {(isPlaying || isPaused) && (
              <button onClick={() => doAction(() => (session as any).locked ? apiUnlockQuizSession(sessionId!) : apiLockQuizSession(sessionId!), 'lock')}
                className="flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-bold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors">
                {(session as any).locked ? <><Unlock className="w-4 h-4" />{t('quiz.unlock')}</> : <><Lock className="w-4 h-4" />{t('quiz.lock')}</>}
              </button>
            )}
          </div>
        </div>
      ...
      {isLobby ? (
        <div className="mt-8 flex flex-col items-center pb-20">
          <div className="w-full flex items-center justify-between mb-8 px-4">
            <div className="flex items-center gap-3 bg-white dark:bg-slate-800 px-6 py-3 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
              <Users className="w-6 h-6 text-slate-800 dark:text-white" />
              <span className="text-2xl font-extrabold text-slate-900 dark:text-white">{participants.length}</span>
            </div>
            
            <div className="flex items-center gap-3">
              <button onClick={() => doAction(() => (session as any).locked ? apiUnlockQuizSession(sessionId!) : apiLockQuizSession(sessionId!), 'lock')}
                className="flex items-center justify-center p-3 rounded-xl text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 transition-colors shadow-sm w-12 h-12">
              ...
              <button onClick={() => doAction(() => apiStartQuizSession(sessionId!), 'start')} disabled={!!actionLoading || participants.length === 0}
                className="flex items-center justify-center gap-2 px-8 py-3 rounded-xl font-extrabold text-white text-xl transition-all disabled:opacity-50 active:scale-[0.98]"
                style={{ backgroundColor: 'var(--kahoot-green)', boxShadow: '0 4px 0 #1b6308' }}>
                {t('quiz.startGame')}
              </button>
            </div>
          </div>

          <div className="flex flex-wrap items-center justify-center gap-3 w-full max-w-5xl px-4">
            {participants.length === 0 ? (
              <div className="mt-10 animate-pulse">
                <span className="text-2xl font-extrabold text-slate-500 uppercase tracking-widest" style={{ fontFamily: 'Montserrat, sans-serif' }}>
              ...
              participants.map(p => (
                <div key={p.participantId} 
                     className="group relative px-6 py-2.5 rounded-md flex items-center gap-2 transform transition-transform hover:scale-110 animate-in fade-in zoom-in duration-300" 
                     style={{ backgroundColor: 'var(--kahoot-purple)', boxShadow: '0 4px 0 rgba(0,0,0,0.2)' }}>
                  <span className="text-white font-extrabold text-lg sm:text-xl tracking-wide truncate max-w-[200px]" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                    {p.participantName}
                  </span>
                  <button onClick={() => { if (confirm(t('quiz.kickConfirm'))) apiKickParticipant(sessionId!, p.participantId) }}
                    className="absolute -top-2 -right-2 bg-red-500 hover:bg-red-600 rounded-full p-1.5 opacity-0 group-hover:opacity-100 transition-opacity transform hover:scale-110 shadow-lg cursor-pointer">
