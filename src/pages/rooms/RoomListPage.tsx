      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 font-medium ml-1">
            {rooms.filter(r => r.status === 'active').length} {t('rooms.activeRooms', 'Live Sessions')}
          </p>
        </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2">
                    <Radio className={`w-4 h-4 ${room.status === 'active' ? 'text-emerald-500 animate-pulse' : 'text-slate-400 dark:text-slate-500'}`} />
                    <span className={`px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded border ${room.status === 'active' ? 'bg-emerald-100 text-emerald-700 border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-400 dark:border-emerald-800' : 'bg-slate-100 text-slate-600 border-slate-200 dark:bg-slate-800 dark:text-slate-400 dark:border-slate-700'}`}>
                      {room.status}
                    </span>
                  </div>
