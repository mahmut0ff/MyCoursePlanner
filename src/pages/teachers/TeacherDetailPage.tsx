...
import { orgGetTeachers } from '../../lib/api';
import { apiGetTeacherProfile } from '../../lib/api';
import { ArrowLeft, Mail, Calendar, BookOpen, Briefcase, Link2, UserPlus } from 'lucide-react';
      ...
      <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden mb-6">
        ...
        <div className="px-6 pb-6 -mt-10">
          ...
          <div className="flex items-center gap-3">
            <span className="text-[10px] px-2 py-0.5 bg-violet-500/10 text-violet-500 rounded-full font-medium">{t('org.teachers.roleTeacher')}</span>
            {teacher.createdAt && (
              <span className="text-[10px] text-slate-400 flex items-center gap-1"><Calendar className="w-3 h-3" />{t('org.users.joined')}: {new Date(teacher.createdAt).toLocaleDateString()}</span>
            )}
          </div>
        </div>
      </div>
