...
const STATUS_CONFIG: Record<VacancyApplicationStatus, { icon: React.ReactNode; color: string; label: string }> = {
  pending: { icon: <HourglassIcon className="w-3.5 h-3.5" />, color: 'bg-amber-500/10 text-amber-600', label: 'На рассмотрении' },
  viewed: { icon: <Eye className="w-3.5 h-3.5" />, color: 'bg-blue-500/10 text-blue-600', label: 'Просмотрено' },
  accepted: { icon: <CheckCircle className="w-3.5 h-3.5" />, color: 'bg-emerald-500/10 text-emerald-600', label: 'Принято' },
  rejected: { icon: <XCircle className="w-3.5 h-3.5" />, color: 'bg-red-500/10 text-red-600', label: 'Отклонено' },
};
          ...
          {applications.map((app) => {
            ...
            return (
              <div key={app.id} onClick={() => navigate(`/vacancies/${app.vacancyId}`)}
                className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-4 cursor-pointer hover:shadow-md transition-all">
